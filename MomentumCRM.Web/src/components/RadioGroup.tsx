  ...
  <AriaRadioGroup {...props} className="flex flex-col gap-2">
    ...
    {options.map((opt) => (
      <RadioField key={opt.value} value={opt.value} className="data-[disabled]:opacity-50">
        <RadioButton className="group flex items-center gap-2 text-sm text-slate-700 hover:cursor-pointer">
          <div className="flex h-4 w-4 items-center justify-center rounded-full border border-slate-300 bg-white transition group-data-[selected]:border-brand-600 group-data-[focus-visible]:ring-2 group-data-[focus-visible]:ring-brand-500/40">
